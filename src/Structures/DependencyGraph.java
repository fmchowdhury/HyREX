...
public class DependencyGraph {
	...
	public DependencyGraph clone() {
		DependencyGraph newNode = new DependencyGraph();
		
		newNode.allNodesByWordIndex = (DepGraphVertex[]) allNodesByWordIndex.clone();
		return newNode;
	}
	 ...
	 * @param origSen
	 */
	public DependencyGraph ( String[][] arrWordAndPosByParser, String[] listAllDepRelations, String[][] arrWordsAndNEs, DependencyTree dt ){		
		
		if ( dt == null)
			dt = new DependencyTree(arrWordAndPosByParser, listAllDepRelations, arrWordsAndNEs);
		...
		for ( int i=0; i<dt.allNodesByWordIndex.length; i++ ){
			...
			allNodesByWordIndex[i].setNEcategory(dt.allNodesByWordIndex[i].getNEcategory());
			...
			allNodesByWordIndex[i].parentWordIndexes.addAll(dt.allNodesByWordIndex[i].getParentsWordIndexes());
			allNodesByWordIndex[i].relNameWithParents.addAll(dt.allNodesByWordIndex[i].getRelNamesWithParents());
			
			allNodesByWordIndex[i].childrenWordIndex.addAll(dt.allNodesByWordIndex[i].getChildrenWordIndexes());
			allNodesByWordIndex[i].relNamesWithChildren.addAll(dt.allNodesByWordIndex[i].getRelNamesWithChildren());
		}
	}
