...
public class SyntacticParser {
	...
	public static String getLemma( String word, String pos){
		return getLemma(word, pos, false);
	}
	 ...
	 * @return
	 */
	public static String getLemma( String word, String pos, boolean useLVG ){
		
	   	 if ( CommonUtility.apiLvgNorm != null && useLVG && pos.toLowerCase().charAt(0) == 'n' ){
			    Vector<LexItem> out = null;
				
				try {
					out = CommonUtility.apiLvgNorm.Mutate(word, false);
				} catch (Exception e) {
					e.printStackTrace();					
				}
				
				if ( out.size() != 0 )
					return ((LexItem)out.elementAt(out.size()-1)).GetTargetTerm().trim();
		 }
		 else if ( word.matches(".*[a-zA-Z]+.*") && !word.matches(".*\\s+.*") )
	 ...
	 * @return
	 */
	private String extractWordLemmaPoS( String tokenWithPos, boolean useLVG ){
		...
		for( int i=0; i<wordAndPos.length; i++ ){
			 ...
			 */
			sb.append(wordAndPos[i][0]).append("\t").append(getLemma(wordAndPos[i][0], wordAndPos[i][1], useLVG))
	...
	public void separateTokensAndDepRels( String inFileName, String outTokenizeFileName,
			String outDepRelFileName, String inSenIdFileName, boolean isSkipParsing, boolean useLVG ) throws Exception{
		...
		for ( int i=0; i<listOfMultilineInput.size(); i++ ) {
			...
			FileUtility.writeInFile(outTokenizeFileName, extractWordLemmaPoS(listOfMultilineInput.get(i).get(0), useLVG), true);			
	...
	public void callStanfordParser( String inSenFileName, String outFileName, 
		...
		String[] options = new String[]{"-maxLength", "1000",   
					"-outputFormat", "wordsAndTags,penn,typedDependenciesCollapsed"};
		 
		//if ( grammarFileForStanfordParser == null )
			grammarFileForStanfordParser = "edu/stanford/nlp/models/lexparser/englishPCFG.ser.gz";
		
		LexicalizedParser lp = LexicalizedParser.loadModel(grammarFileForStanfordParser, options);
		...
		while (( line = input.readLine()) != null){
	    	...
	    	if ( !line.isEmpty() ){
	    		...
	    		try{
	    		    ...
	    		    List<? extends HasWord> tmp = toke.tokenize();
	    		    List<Word> sentence = new ArrayList<Word>();
	    		    
	    		    /*
	    			 * Check whether any tokens misses POS tags, e.g. "2 1\//CD"
	    			 * In that case, use the tokens to get new output
	    			 */
	    		    for ( int i=0; i<tmp.size(); i++ ) {
	    		    	String word = ((Word) tmp.get(i)).word();
	    		    	String[] str = word.trim().split("\\s+");
	    		    	for ( int w=0; w<str.length; w++ ) {
	    		    		sentence.add(new Word(str[w]));
	    		    	}
	    		    }
		...
		// TODO: use the other read method which can handle empty dependency output
