...
/*
		...
		for ( int i=0; i<corpus.length; i++ ) {
			String path = "/media/Study/data/5_PPI_corpora/" + corpus[i] + "/" + corpus[i] ;
			mergePsgAndDepOutput( path + ".parsed.bllip", path + ".parsed.bllip.dep", 
				path + ".parsed.bllip.complete");
		}
		*/
		
		
		String path = "../../data/SemEval2013/task-9-ddi/test_data/task9.2/" ;
		mergePsgAndDepOutput( path + "test.parsed.bllip", path + "test.parsed.bllip.dep", 
		...
		for ( int i=0; i<listOfMultilineInput.size(); i++ ) {
			...
			if ( hasPsgTree ) {
				...
				listOfSentence.get(senNo).psgTree = new PhraseStructureTree(str, 
						listOfSentence.get(senNo).arrWordAndPosByParser, null);
				i++;
			}
			
			if ( hasDepTree )
				listOfSentence.get(senNo).depGraph = new DependencyGraph(listOfSentence.get(senNo).arrWordAndPosByParser,
					DataStrucUtility.listToStringArray(listOfMultilineInput.get(i)), new String[0][0], listOfSentence.get(senNo).depTree);
